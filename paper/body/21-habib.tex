...
This algorithm is described fully in \cite{habib}.

The algorithm is divided into two phases. The first phase is creating the factorizing permutation (which will be defined in a moment) and the second phase consists of sweeping the permutation in order to check if graph is a cograph.
...
\begin{defi}[Refinement rule 1]\label{habib:refrule1}
    Let $\Part P = (\Part X_1, \ldots, \Part X_k)$ be a partition of $\V(G)$ and $\Part X_i$ be a part in this partition. Pick vertex $v \in \Part X_i$ as a pivot and refine $\Part P$ into
    \[
...
\begin{defi}[Refinement rule 2]\label{habib:refrule2}
    Let $\Part P = (\Part X_1, \ldots, \Part X_k)$ be a partition of $\V(G)$ and $\Part X_i$ be a part in this partition. Let $v \not\in \Part X_i$ be a vertex such that $\exists_{u,u' \in \Part X_i} \{u,v\} \in \E(G) \land \{u',v\} \not\in \E(G)$. Now refine $\Part P$ into
    \[
        (\Part X_1, \ldots, \Part X_{i-1}, \Nbar(v) \cap \Part X_i, \N(v) \cap \Part X_i, \Part X_{i+1}, \ldots, \Part X_k).
    \]
\end{defi}

Proofs that these refinements produce permutations compatible with factorizing permutation can be found in \cite{habib} and \cite{habib2}. Pseudo code of implementation of the first rule is presented in \ref{habib:refrule1impl}.
...
\begin{function}
    ...
    \Begin{
        \Return{$(\Part X_1, \ldots, \Nbar_G(O) \cap \Part X_O, \{O\}, \N_G(O) \cap \Part X_O, \ldots, \Part X_k)$}
    }
\end{function}
...
This algorithm runs in $O(|\V(G)| + |\E(G)|)$ if implemented correctly.
...
Since we will touch each neighborhood $O(1)$ times, it follows that the whole algorithm runs in $O\left(\sum_{v\in V} \abs{\N(v)}\right) = O(|\V(G)| + |\E(G)|)$.